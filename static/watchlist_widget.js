...
(function() {
    ...
    const DEFAULT_CONFIG = {
        gpgPublicKey: null, // Will be fetched from server
        apiEndpoint: window.WatchlistConfig?.apiEndpoint || 'https://watchlist.layer55.eu/api/watchlist',
        formTitle: window.WatchlistConfig?.formTitle || 'Join Our Waitlist',
        formDescription: window.WatchlistConfig?.formDescription || 'Sign up to get early access to our product!',
        submitButtonText: window.WatchlistConfig?.submitButtonText || 'Join Waitlist'
    };
    ...
    async function fetchGpgKey() {
        try {
            ...
            const baseUrl = `${apiUrl.protocol}//${apiUrl.hostname}:${apiUrl.port || (apiUrl.protocol === 'https:' ? '443' : '80')}`;
            const gpgKeyUrl = `${baseUrl}/gpg-key`;
            
            console.log('Fetching GPG key from:', gpgKeyUrl);
            ...
            if (!keyData || keyData.trim() === '') {
                throw new Error('Received empty GPG key data');
            }
    ...
    async function encryptData(data) {
        try {
            // Fetch GPG key if not already available
            let gpgKey = config.gpgPublicKey;
            if (!gpgKey) {
                gpgKey = await fetchGpgKey();
                // Cache the key for future use
                config.gpgPublicKey = gpgKey;
            }
            
            const publicKey = await openpgp.readKey({ armoredKey: gpgKey });
